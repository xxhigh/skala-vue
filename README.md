...
- 중앙 검색 버튼이 검색창으로 확장되는 도시 자동완성과 최근 검색 도시 최대 5개 저장
- 상세 화면에서 지역 즐겨찾기 추가·삭제 및 빠른 전환
- 섭씨·화씨 전환 및 라이트·다크 테마 지원
- 표시 중인 도시의 현지 시각을 `HH:MM` 형식으로 매분 갱신
- 현재 날씨와 주야간 상태에 맞는 랜딩 배경 영상
- 랜딩 헤더에서 배경 영상 재생·일시정지 제어
...
새로운 날씨 유형에 대응하는 영상이 없으면 `sunny_0.mp4`를 사용합니다. MP4 파일은 저장소의 `*.mp4` 무시 규칙에 포함되므로 새 환경에는 별도로 복사해야 합니다. 영상 재생이 실패하거나 사용자가 동작 줄이기·데이터 절약을 설정한 경우 테마별 `bg_white.jpg` 또는 `bg_dark.jpg`가 표시됩니다.

## 프로젝트 구조

```text
src/
├── components/weather-app/  # 헤더, 즐겨찾기, 요약, 예보 및 공통 아이콘
...
자동화된 테스트 프레임워크는 아직 구성되어 있지 않습니다. 변경 후 최소한 `npm run lint`와 `npm run build`를 실행하세요. UI 변경 시 320px부터 데스크톱까지 확인하고 위치 거부, 도시 검색, 현지 시각, 영상 제어, 테마·단위 전환, 즐겨찾기 및 상세 이동을 직접 검증하세요.
